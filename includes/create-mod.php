...
$postImg = '../images/mod-images/'.basename($_FILES["modImageUpload"]["name"]);

$postTitle = $_POST['MTitle'];
$postCategory = $_POST['MCat'];
$downloadLink = $_POST['MLink'];
$postAuthor = $_POST['MAuthor'];
$postBody = $_POST['MDesc'];
$metaPostDesc = 'Download the '.$postTitle.' FS 19 mod here!';

$fileNameLSS = strtolower($postTitle);
$uFilenameSS = str_replace(" ","-",$fileNameLSS);
$uFilename = $uFilenameSS."-fs19";
...
$target_dir = "../images/mod-images/";
$target_file = $target_dir . basename($_FILES["modImageUpload"]["name"]);
$uploadOk = 1;
$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

if(isset($_POST["submit"])) {
  $check = getimagesize($_FILES["modImageUpload"]["tmp_name"]);
  if($check !== false) {
...
if ($_FILES["modImageUpload"]["size"] > 2000000) {
...
&& $imageFileType != "gif" ) {
    header("Location: ../admin/dashboard?uploaded=false&reason=filetype");
    $uploadOk = 0;
}
...
if ($uploadOk == 0) {
  ...
  if (move_uploaded_file($_FILES["modImageUpload"]["tmp_name"], $target_file)) {
    header("Location: ../admin/dashboard?uploaded=true&murl=".$uFilename);
  } else {
...
$newfile = fopen("../mods/".$uFilename.".php", "w") or die("Unable to open file!");

$HTMLCode = '
<!-- Keywords + SEO -->
<title>FS19 '.$postTitle .' - BestFSMods</title>
...
<!-- Social Media -->
<meta name="twitter:title" content="FS19 '.$postTitle .' - BestFSMods">
<meta name="twitter:description" content="'.$metaPostDesc.'">
<meta name="twitter:image" content="'.$postImg.'">
<meta name="twitter:card" content="summary_large_image">

<meta property="og:title" content="FS19 '.$postTitle.' - BestFSMods">
  ...
  <main role="main" class="container">
    ...
    <div class="row justify-content-center">
      <p class="lead" style="font-size: 18px; margin-left: 10px; padding-right: 10px; border-right: 1px solid #dee2e6;">
        Author: '.$postAuthor.'</p>
      <p class="lead" style="font-size: 18px; margin-left: 10px; padding-right: 10px; border-right: 1px solid #dee2e6;">
        '.$monthName.' '.$postDay.', 2020</p>
      <p class="lead" style="font-size: 18px; margin-left: 10px; margin-right: 10px;"><a
          href="/categories/'.$postCategory.'" style="text-decoration: none;">FS 19 '.$postCategory.'</a></p>
    </div>
    <img src="'.$postImg.'" alt="'.$postTitle.' FS19" class="img-fluid mx-auto d-block">
...
<!-- ad was here -->
      ...
      <div class="col-md-9 mr-0 border-left mb-4">
        ...
        <div class="center" style="margin-top: 15px">
          <p class="lead mx-auto">Credit: '.$postAuthor .'</p>
        </div>
        <div style="margin: auto; margin-top: 15px; margin-bottom: 10px;">
<!-- ad was here -->
        </div>
        <div class="center">
          <a href="'.$downloadLink .'" target="_blank"><button class="btn btn-primary btn-lg mx-auto border-0"
